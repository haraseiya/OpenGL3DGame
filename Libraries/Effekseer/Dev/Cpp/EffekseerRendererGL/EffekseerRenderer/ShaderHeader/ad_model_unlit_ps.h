...
vec4 _main(PS_Input Input)
{
    ...
    vec3 _407 = Output.xyz * CBPS0.fEmissiveScaling.x;
...
vec4 _main(PS_Input Input)
{
    ...
    vec3 _480 = Output.xyz * CBPS0.fEmissiveScaling.x;
    ...
    vec3 _578 = mix(CBPS0.fEdgeColor.xyz * CBPS0.fEdgeParameter.y, Output.xyz, vec3(ceil((Output.w - advancedParam.AlphaThreshold) - CBPS0.fEdgeParameter.x)));
...
void main()
{
    ...
    vec4 _614 = _main(Input);
    _entryPointOutput = _614;
}
...
highp vec4 _main(PS_Input Input)
{
    ...
    if (CBPS0.fFalloffParameter.x == 1.0)
    {
        ...
        if (CBPS0.fFalloffParameter.y == 0.0)
        {
            highp vec3 _367 = Output.xyz + FalloffBlendColor.xyz;
            Output = vec4(_367.x, _367.y, _367.z, Output.w);
        }
        else
        {
            if (CBPS0.fFalloffParameter.y == 1.0)
            {
                highp vec3 _380 = Output.xyz - FalloffBlendColor.xyz;
                Output = vec4(_380.x, _380.y, _380.z, Output.w);
            }
            else
            {
                if (CBPS0.fFalloffParameter.y == 2.0)
                {
                    highp vec3 _393 = Output.xyz * FalloffBlendColor.xyz;
                    Output = vec4(_393.x, _393.y, _393.z, Output.w);
                }
            }
        }
    ...
    highp vec3 _407 = Output.xyz * CBPS0.fEmissiveScaling.x;
    Output = vec4(_407.x, _407.y, _407.z, Output.w);
    if (Output.w <= max(0.0, advancedParam.AlphaThreshold))
    {
        discard;
...
highp vec4 _main(PS_Input Input)
{
    ...
    if (CBPS0.fFalloffParameter.x == 1.0)
    {
        ...
        if (CBPS0.fFalloffParameter.y == 0.0)
        {
            highp vec3 _440 = Output.xyz + FalloffBlendColor.xyz;
            Output = vec4(_440.x, _440.y, _440.z, Output.w);
        }
        else
        {
            if (CBPS0.fFalloffParameter.y == 1.0)
            {
                highp vec3 _453 = Output.xyz - FalloffBlendColor.xyz;
                Output = vec4(_453.x, _453.y, _453.z, Output.w);
            }
            else
            {
                if (CBPS0.fFalloffParameter.y == 2.0)
                {
                    highp vec3 _466 = Output.xyz * FalloffBlendColor.xyz;
                    Output = vec4(_466.x, _466.y, _466.z, Output.w);
                }
            }
        }
    ...
    highp vec3 _480 = Output.xyz * CBPS0.fEmissiveScaling.x;
    Output = vec4(_480.x, _480.y, _480.z, Output.w);
    ...
    highp vec3 _578 = mix(CBPS0.fEdgeColor.xyz * CBPS0.fEdgeParameter.y, Output.xyz, vec3(ceil((Output.w - advancedParam.AlphaThreshold) - CBPS0.fEdgeParameter.x)));
    Output = vec4(_578.x, _578.y, _578.z, Output.w);
    return Output;
}
